...
import net.umpay.mailbill.api.model.cmsbbank.CmsbBillView;
 ...
 */
@Service
public class CmsbConvertImpl implements IConvert{

	@Override
	public int getBillType() {
		return MailBillTypeConstants.BILL_TYPE_CMSB;
	}

	@Override
	public CmsbBillView convertEntityAndView(
		...
		CmsbBillView cmsbBillView = new CmsbBillView();
		...
		for(IntegrationDetailEntity detailEntity : detailEntities){
			...
			int tourismPoints = detailEntity.getTourismPoints();
			if (-1 != tourismPoints){
				billView2.setTourismPoints(tourismPoints);
			}
		...
		cmsbBillView.setBillView(billView);
		cmsbBillView.setBalanceBillViews(balanceBillViews);
		cmsbBillView.setMonthBillViews(monthBillViews);
		cmsbBillView.setIntegrationBillView(integrationBillView);
		return cmsbBillView;
	}
