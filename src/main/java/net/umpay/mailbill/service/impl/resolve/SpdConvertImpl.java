...
import net.umpay.mailbill.api.model.spdbank.SpdBillView;
 ...
 */
@Service
public class SpdConvertImpl implements IConvert {

	@Override
	public int getBillType() {
		return MailBillTypeConstants.BILL_TYPE_SPD;
	}

	@Override
	public SpdBillView convertEntityAndView(
		...
		SpdBillView spdBillView = new SpdBillView();
		...
		double cashRmbAdvanceLimit = billCycleInfoEntity
				.getCashRmbAdvanceLimit();
		if (-1 != cashRmbAdvanceLimit) {
			billView.setCashRmbAdvanceLimit(Double
					.toString(cashRmbAdvanceLimit));
		}
		...
		double pastDueAmount = billCycleInfoEntity.getPastDueAmount();
		if (-1 != pastDueAmount) {
			billView.setPastDueAmount(Double.toString(pastDueAmount));
		}
		...
		for (BillBankMonthDetailEntity bankMonthDetailEntity : bankMonthDetailEntities) {
			...
			if (null != merchandiseDate) {
				monthBillView.setMerchandiseDate(DateUtil
						.getFormatDate(merchandiseDate));
			}
			...
			String merchandiseDetail = bankMonthDetailEntity
			...
			double originalTransAmount = bankMonthDetailEntity
					.getOriginalTransAmount();
		...
		spdBillView.setBillView(billView);
		spdBillView.setBalanceBillViews(balanceBillViews);
		spdBillView.setMonthBillViews(monthBillViews);
		spdBillView.setIntegrationBillView(integrationBillView);
		return spdBillView;
	}
